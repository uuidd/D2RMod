...
if (config.demoSelect !== '' || config.demoSelect !== 'def') {
  ...
  demo1.push(normalText2('回城书 + 辨识卷轴 + 红药', '尼拉塞克的神殿/皮叔', 'Red'));
...
// demo2 模拟交易
  ...
  demo2.push(bigTitle('注意'));
  demo2.push(normalText('奸商机制，暗金/绿装兑换符文所得符文号数减一。笑', 'Red', 2));
  demo2.push(normalText('例乔丹之石换符文，得到27#而不是28#', 'Red', 2));
  demo2.push(normalText('数字加＃表示符文编号，为符文(需求等级-3)/2，除了15#符文海尔', 'Red', 2));
  demo2.push(normalText('以下都在盒子(方块)中合成，放入装备点击转化会变成符文', 'Red', 2));
...
// demo3 模组说明
  ...
  demo3.push(bigTitle('DEMOMOD其他功能'));
  demo3.push(normalText('MOD说明百科，就是你现在看的这个，笑'));
  demo3.push(normalText('大箱子16X16，7页无缩放，需自行替换共享仓库文件，比缩放的大箱子更美观'));
  demo3.push(normalText('大背包13X8，项链/戒指在鞋子上面，还算美观'));
  demo3.push(normalText('大盒子10X10，跟初始箱子一样大，同样无缩放且方方正正，百科可由此右侧点击弹出'));
  demo3.push(normalText('佣兵全装备，可带两个戒指'));
  demo3.push(normalText('不太迷你盒子10X10由右下角点击弹出，但会覆盖装备栏，但单独点击背包不覆盖'));
  // demo3.push(normalText('超强白装一定是15ed，非超强白装全是满防御和攻击力，但超强白装掉率没改'));
  demo3.push(normalText('死灵法师的召唤物可以穿过彼此，不再卡位，a2洞窟没那么遭罪了，纯招死灵增强'));
  demo3.push(normalText('删除技能的强制冷却时间，冰法/元素德等加强'));
  demo3.push(normalText('手工品的固定词条满变量，20fcr项链更好做了'));
  demo3.push(normalText('暗金装备满变量，不改爆率暗金太难打'));
  demo3.push(normalText('符文之语满变量，蓝色金色不做更改，保留暗黑2装备精华的部分'));
  ...
  demo3.push(normalText('关底boss bug杀适配2.6/2.7 邪恶区域'));
  ...
  demo3.push(normalText('buff结束会有女声提醒，防止突然暴毙'));
  ...
  demo3.push(normalText('投掷武器堆叠数量提升至500，省得频繁修复，最多511没法更多了'));
  ...
  demo3.push(normalText('一键点击角色进入地狱难度功能，退出游戏鼠标不用移动点击就能快速刷刷刷'));
  demo3.push(normalText('无法进入地狱的角色点击有提示且不会闪退，注意此功能无文本提示（文本太丑了）'));
  demo3.push(normalText('添加彩色词条，集中关注重点词条'));
  demo3.push(normalText('静音光环声音，不会那么吵了'));
  demo3.push(normalText('怪物禁掉精力/解毒/融冰药剂、箭矢/弩矢、钥匙、回城/辨识卷轴、爆炸/毒气瓶'));
  demo3.push(normalText('基德出售 +1 传送技能特大咒符，可用可不用'));
  demo3.push(normalText('基德出售近战范围伤害特大咒符，可用可不用'));
  demo3.push(normalText('隐藏丑丑的绿帽子'));
  demo3.push(normalText('加快后期升级速度，类似升级原版虚线，不会一刀99'));
  demo3.push(normalText('扩大邪恶区域出现场景数量，邪恶区域将一次性循环很多张地图'));
  demo3.push(normalText('怪物脚底加光源，避免眼瞎看不见怪'));
  demo3.push(normalText('显示物品等级，物品后面会显示(xx)，打孔/做手工/刷新物品用得到'));
  // demo3.push(bigTitle('jgmod的功能'));
  // demo3.push(normalText('https://www.caimogu.cc/post/77223.html'));
...
// demo4 可选功能
  ...
  demo4.push(normalText('DemoMod 反馈群【661975153】', 'Yellow', 2));
  demo4.push(normalText('【小背包版本】于Q群下载', 'Yellow'));
  demo4.push(normalText('小背包版本我的自用版，我喜欢原版', 'Yellow'));
